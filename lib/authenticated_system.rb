module AuthenticatedSystem
  protected
    ...
    def self.included(base)
      base.send :helper_method, :current_sessions, :logged_in?, :authorized? if base.respond_to? :helper_method
    end
    ...
    def login_from_basic_auth
      authenticate_with_http_basic do |login, password|
        self.current_sessions = Sessions.authenticate(login, password)
      end
    end
    ...
    # for the paranoid: we _should_ be storing sessions_token = hash(cookie_token, request IP)
    ...
    def logout_keeping_session!
      ...
      @current_sessions.forget_me if @current_sessions.is_a? Sessions
      @current_sessions = false     # not logged in, and don't do it for me
      kill_remember_cookie!     # Kill client-side auth cookie
      session[:sessions_id] = nil   # keeps the session but kill our variable
    ...
    def valid_remember_cookie?
      return nil unless @current_sessions
      (@current_sessions.remember_token?) && 
        (cookies[:auth_token] == @current_sessions.remember_token)
    end
    
    # Refresh the cookie auth token if it exists, create it otherwise
    def handle_remember_cookie!(new_cookie_flag)
      return unless @current_sessions
      case
      when valid_remember_cookie? then @current_sessions.refresh_token # keeping same expiry date
      when new_cookie_flag        then @current_sessions.remember_me 
      else                             @current_sessions.forget_me
      end
    ...
    def send_remember_cookie!
      cookies[:auth_token] = {
        :value   => @current_sessions.remember_token,
        :expires => @current_sessions.remember_token_expires_at }
    end
